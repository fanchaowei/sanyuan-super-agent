...
import { countTools } from './mock/tools'
...
// 注册模拟的 hook
registerMockSecurityHook(hookPipeline, config)
...
const cronService = new CronService({ baseDir: config.cron.dataDir });
...
// ── Sub-Agent ────────────────────────────────────────
const agentRegistry = new SubAgentRegistry({ maxSpawnDepth: config.agents.maxSpawnDepth, maxConcurrent: config.agents.maxConcurrent });
