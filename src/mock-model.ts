...
function makeToolCallChunks(intents: ToolCallIntent[], prompt: any[]): any[] {
  ...
  // Mock model declares the AI SDK v2 protocol, whose finishReason is a string.
  chunks.push({ type: 'finish', finishReason: 'tool-calls', usage: makeUsage(prompt) });
  return chunks;
}

export function createMockModel() {
  return {
    // This mock implements the AI SDK v2 language-model contract. In v2,
    // finishReason is a string; AI SDK 6 converts it to the v3 object shape.
    ...
    async doStream({ prompt }: any) {
      ...
      if (text.includes('测试重试') || text.includes('test retry')) {
        ...
        const chunks: any[] = [
          ...
          { type: 'text-end', id },
          { type: 'finish', finishReason: 'stop', usage: makeUsage(prompt) },
        ];
